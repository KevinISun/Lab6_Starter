  ...
  time {
	...
	set data(data) {
		...
		article.innerHTML = `
			...
			<div class="rating">
				<span class="rating">${data.rating !== undefined ? data.rating : 'No Reviews'}</span>
				<img src="assets/images/icons/${data.rating ? Math.round(data.rating) : 0}-star.svg" alt="${data.rating ? Math.round(data.rating) : 0} stars">
				<span>(${data.numRatings !== undefined ? data.numRatings : 0})</span>
			</div>
